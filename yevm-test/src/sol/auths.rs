...
use yevm_base::{Acc, Int, acc, int};
use yevm_core::{Call, Head, Tx, state::Account};
...
fn encode_execute(signer: &Acc, target: &Acc, data: &[u8], nonce: u64, sig: &[u8; 65]) -> Vec<u8> {
    let sel = yevm_misc::keccak256(b"execute(address,address,bytes,uint256,bytes)");
    let data_pad = data.len().div_ceil(32) * 32;
...
async fn test_meta_tx() -> eyre::Result<()> {
    ...
    let set_sel = yevm_misc::keccak256(b"set(uint256)");
    ...
    let auth_hash = yevm_misc::keccak256(&auth_src.bin_runtime.0);
    let box_hash = yevm_misc::keccak256(&box_src.bin_runtime.0);
